...
function Placeorder() {
    ...
    React.useEffect(() => {
        ...
        console.log(orderDetails.Orderplaced.order)
    }, [id, dispatch])
    return (
        <div style={{ padding: '1rem' }} className='placeOrder_wrapper'>
            <div className="info_wrapper">
                <div style={{ padding: '0 1rem', fontWeight: 'bolder' }} className="orderId">Order Id : {id}</div>

                <div className="shipping_info">
                    <div className='info_head'>
                        Shipping
                    </div>
                    <div className="info">Name: {existingorder?.shippingAddress?.fullname}</div>
                    <div className="info">Address: {existingorder?.shippingAddress?.Address}</div>
                    {
                        existingorder?.isDelivered ? (
                            <div style={{ backgroundColor: 'rgba(154, 205, 50,0.2)' }} className="alert">
                                <CheckCircleOutlineOutlined style={{ color: 'yellowgreen' }} />Delivered
                            </div>
                        ) : (
                            <div style={{ backgroundColor: 'rgba(255, 99, 71, 0.2)' }} className="alert">
                                <WarningRounded color='secondary' />Not Delivered
                            </div>
                        )
                    }
                </div>
                <div className="payment_method">
                    ...
                    <div className="info">Method: <span>{existingorder?.paymentMethod}</span> </div>
                    {
                        existingorder?.isPaid ? (
                            <div style={{ backgroundColor: 'rgba(154, 205, 50,0.2)' }} className="alert">
                                <CheckCircleOutlineOutlined style={{ color: 'yellowgreen' }} />Paid
                            </div>
                        ) : (
                            <div style={{ backgroundColor: 'rgba(255, 99, 71, 0.2)' }} className="alert">
                                <WarningRounded color='secondary' />Not Paid
                            </div>
                        )
                    }
                </div>
            ...
            <div className="ordercalcs_wrapper">
                ...
                <div className="ordercalcs_items">
                    ...
                    <div className="ordercalcs_amount">${Number(existingorder?.itemsPrice).toFixed(2)}</div>
                </div>
                <div className="ordercalcs_items">
                    <div className="key">Shipping</div>
                    <div className="ordercalcs_amount">${Number(existingorder?.shippingPrice).toFixed(2)}</div>
                </div>
                <div className="ordercalcs_items">
                    <div className="key">Tax</div>
                    <div className="ordercalcs_amount">${Number(existingorder?.taxprice)}</div>
                </div>

                <div className="ordercalcs_items ordercalcs_total">
                    <div className="key">Order Total</div>
                    <div className="ordercalcs_amount">${Number(existingorder?.totalPrice)}</div>
                </div>
